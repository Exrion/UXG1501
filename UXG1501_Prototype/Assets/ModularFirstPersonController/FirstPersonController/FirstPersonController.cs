...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            ...
            if (gamepad != null)
                yaw = transform.localEulerAngles.y + gamepad.rightStick.x.value * mouseSensitivity / 2;

            if (!invertCamera)
            {
                float diff = mouseSensitivity * Input.GetAxis("Mouse Y");
                if (gamepad != null)
                    diff = mouseSensitivity * gamepad.rightStick.y.value / 2;
                pitch -= diff;
            }
            else
            {
        ...
        if (enableZoom)
        {
            ...
            if ((Input.GetKeyDown(zoomKey) || gamepad != null ? gamepad.leftTrigger.isPressed : false) && !holdToZoom && !isSprinting)
            {
                if (!isZoomed)
                {
                    isZoomed = true;
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && (Input.GetKey(sprintKey) || gamepad != null ? gamepad.rightShoulder.isPressed : false) && sprintRemaining > 0f && !isSprintCooldown)
            {
